...
def runtime_janus_etc_dir(ws_port):
    # Multiple moonraker-obico instances on the same host share the same codebase (unlike
    # OctoPrint-Obico, where each OctoPrint instance typically has its own plugin install).
    # Keying the runtime config dir off the (per-instance) ws_port keeps concurrently running
    # instances from clobbering each other's janus.jcfg / janus.plugin.streaming.jcfg files.
